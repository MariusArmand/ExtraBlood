...
void actKillDude(int nKillerSprite, spritetype *pSprite, DAMAGE_TYPE damageType, int damage)
{
    ...
    switch (pSprite->type) {
        ...
        case kDudeCultistShotgun: {
            ...
            else // extrablood code
            {
                ...
                {
                    PLAYER *pPlayer = &gPlayer[pKillerSprite->type - kDudePlayer1];
                    if (!pPlayer->hasDoubleWeapon[kWeaponShotgun])
                    {
                        // as long as we don't have double shotguns increase the chance to drop a shotgun
                        if (nRand <= 50) actDropObject(pSprite, kItemWeaponSawedoff);
    ...
    if (damageType == kDamageExplode)
    {
        ...
        {
            // for default, spawn more blood on explosion
            switch (pSprite->type) {
                case kDudePhantasm:
                case kDudeHand:
                case kDudeSpiderBrown:
            ...
            if (pSprite->statnum == kStatDude)
            {
                ...
                else // extrablood code
                {
                    ...
                    default:
                        {
                            #ifdef NOONE_EXTENSIONS
                            if (pSprite->type == kDudeModernCustom && DudeIsSmall(pSprite))   
                            {
                                for (int i = 0; i < pVectorData->bloodSplats; i++)
                                    if (Chance(pVectorData->splatChance))
                                        fxSpawnBlood(pSprite, pVectorData->dmg<<4);       
                            ...
                            else
                            {
                                ...
                                else // shooter is not a player
                                {
                                    for (int j = 0; j < bloodSplats; j++) 
                                    {
                                        ...
                                        case kDudeGargoyleFlesh:
                                        case kDudeGargoyleStone:
                                            if (vectorType == kVectorGargSlash && Chance(0x4000))
                                                fxSpawnBlood(pSprite, pVectorData->dmg<<4);
                                            break;
                                        default:
                                            fxSpawnBlood(pSprite, pVectorData->dmg<<4);
